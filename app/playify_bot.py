...
def run_bot(status_queue, log_queue, command_queue):
    ...
    async def create_controller_embed(bot, guild_id):
        ...
        if len(tracks_to_display) > 1:
            ...
            description_lines.append(
                get_messages("controller.queue_empty.title", guild_id)
            )
    ...
    class SeekView(View):
        ...
        def __init__(self, interaction: discord.Interaction):
            ...
            self.forward_button.label = get_messages(
                "seek.button.forward", self.guild_id
            )
            ...
            try:
                ...
                if not song:
                    fail_message = get_messages(
                        "lyrics.error.not_found.description", self.guild_id
    ...
    class KaraokeRetryModal(discord.ui.Modal, title="Refine Karaoke Search"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if not lrc:
                fail_message = get_messages("karaoke.retry.fail", self.guild_id).format(
                    query=new_query
                )
                await interaction.followup.send(
    ...
    class KaraokeRetryView(discord.ui.View):
        def __init__(
            ...
            self.retry_button.label = get_messages(
                "karaoke.retry_button", self.guild_id
            )
        ...
        try:
            ...
            if not song:
                ...
                error_desc = get_messages(
                    "lyrics.error.not_found.description", guild_id
                            ...
                            try:
                                if not progress_message and music_player.text_channel:
                                    initial_embed = Embed(
                                        ...
                                        description=get_messages(
                                            "autoplay.loading_description", guild_id
                                        ).format(
                                ...
                                if progress_message and added_count > 0:
                                    ...
                                    final_embed.description = get_messages(
                                        "autoplay.finished_description", guild_id
    ...
    async def karaoke(interaction: discord.Interaction):
        ...
        async def proceed_with_karaoke():
            ...
            if not lyrics_lines:
                ...
                error_desc = get_messages(
                    "karaoke.not_found_description", guild_id
    ...
    async def skip(
        ...
        if music_player.loop_current:
            ...
            description_text = get_messages(
                "player.replay.success_desc", guild_id
            ).format(title=title, url=url)
            embed = Embed(
    ...
    async def search(interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            if not search_results:
                embed = Embed(
                    description=get_messages("search.no_results", guild_id).format(
                        query=query
                    ),
                    color=0xFF9AA2 if is_kawaii else discord.Color.red(),
                )
    ...
    async def allowlist(
        ...
        if channels:
            ...
            embed = discord.Embed(
                description=get_messages(
                    "setup.allowlist.set_success", guild_id
                ).format(channels=channel_mentions),
                color=0xB5EAD7 if is_kawaii else discord.Color.green(),
            )
            await interaction.response.send_message(
                embed=embed, ephemeral=True, silent=True
    ...
    async def global_interaction_check(interaction: discord.Interaction) -> bool:
        ...
        description_text = get_messages(
            "command.restricted_description", guild_id
